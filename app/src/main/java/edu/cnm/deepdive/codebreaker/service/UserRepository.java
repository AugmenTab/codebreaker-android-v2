...
public class UserRepository {
  ...
  public Single<User> getCurrentUser() {
    return Single.fromCallable(signInService::getAccount).flatMap((account) ->
        webService.getProfile(getBearerToken(account.getIdToken()))
            .flatMap((user) -> userDao.selectByOauthKey(account.getId())
                .switchIfEmpty(userDao.insert(user).map((id) -> {
                      user.setId(id);
                      return user;
                    })
                )
            ).flatMap((user) -> userDao.update(user).map((numRecords) -> user)
        )
